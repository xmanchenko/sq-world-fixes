...
function centaur_warrunner_hoof_stomp_lua:OnSpellStart()
	...
	local abil = self:GetCaster():FindAbilityByName("npc_dota_hero_centaur_int11")
	if abil ~= nil then
		damage = damage + self:GetCaster():GetIntellect()
	end

	local abil = self:GetCaster():FindAbilityByName("npc_dota_hero_centaur_int7")
	if abil ~= nil then
		stun_duration = stun_duration + 5
	end
