LinkLuaModifier("modifier_death_ward_oaa", "heroes/hero_witch_doctor/death_ward/death_ward.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_death_ward_hidden_oaa", "heroes/hero_witch_doctor/death_ward/death_ward.lua", LUA_MODIFIER_MOTION_NONE)

if witch_doctor_death_ward == nil then
    witch_doctor_death_ward = class({})
end

function witch_doctor_death_ward:IsStealable()
  return true
end

function witch_doctor_death_ward:OnSpellStart()
--   local unit_name = "npc_dota_witch_doctor_death_ward" -- vanilla death ward unit doesn't work for some reason
  ...
  local death_ward = CreateUnitByName("witch_doctor_death_ward", point, true, caster, caster, caster:GetTeamNumber())
...
function witch_doctor_death_ward:OnProjectileHit_ExtraData(target, location, data)
...
function witch_doctor_death_ward:OnUpgrade()
  local caster = self:GetCaster()
...
--function witch_doctor_death_ward:GetCastAnimation()
  --return ACT_DOTA_CAST_ABILITY_4
--end

--function witch_doctor_death_ward:GetChannelAnimation()
	--return ACT_DOTA_VICTORY
--end

function witch_doctor_death_ward:OnChannelFinish(interrupted)
  if self.ward_unit and not self.ward_unit:IsNull() then
...
function witch_doctor_death_ward:ProcsMagicStick()
  return true
end

---------------------------------------------------------------------------------------------------

modifier_death_ward_oaa = class({})
...
function modifier_death_ward_oaa:OnCreated()
  ...
  self.attack_damage = self:GetAbility():GetSpecialValueFor("ward_damage")
  ...
  if IsServer() then
    ...
    -- Change Night Vision
    local night_vision = math.max(800, parent:Script_GetAttackRange() + attack_range_bonus)
...
function modifier_death_ward_oaa:OnIntervalThink()
  ...
  local enemies = FindUnitsInRadius(parent:GetTeamNumber(), parent:GetAbsOrigin(), nil, parent:Script_GetAttackRange(), ability:GetAbilityTargetTeam(), ability:GetAbilityTargetType(), targets_flags, FIND_CLOSEST, false)
  if #enemies > 0 then
    parent:SetIdleAcquire(true)
    parent:SetAcquisitionRange(parent:Script_GetAttackRange())
...
function modifier_death_ward_oaa:GetModifierPreAttack_BonusDamage()
...
witch_doctor_voodoo_switcheroo_oaa = class({})
...
modifier_voodoo_switcheroo_oaa = class({})
...
modifier_death_ward_hidden_oaa = class({})
...
modifier_voodoo_switcheroo_ward_oaa = class({})
