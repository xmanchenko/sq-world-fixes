...
function modifier_luna_moon_glaive_lua_thinker:OnCreated( kv )
	...
	if IsServer() then
		...
		-- set properties
		self:GetParent():SetOrigin( self:GetParent():GetOrigin() + Vector( 0, 0, 100 ) )
		self:GetParent():SetAttackCapability( self.caster:GetAttackCapability() )
		self:GetParent():SetRangedProjectileName( self.caster:GetRangedProjectileName() )
...
function modifier_luna_moon_glaive_lua_thinker:GetModifierProcAttack_Feedback( params )
	...
	-- move thinker
	self:GetParent():SetOrigin( params.target:GetOrigin() + Vector( 0, 0, 100 ) )
