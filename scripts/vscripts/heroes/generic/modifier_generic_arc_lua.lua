...
function modifier_generic_arc_lua:SetJumpParameters( kv )
	...
	local pos_start = self:GetParent():GetOrigin()
	local pos_end = pos_start + self.direction * self.distance
	local height_start = GetGroundHeight( pos_start, self:GetParent() ) + self.start_offset
	local height_end = GetGroundHeight( pos_end, self:GetParent() ) + self.end_offset
	local height_max
