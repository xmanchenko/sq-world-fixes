modifier_jakiro_liquid_fire_intrinsic_lua = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_jakiro_liquid_fire_intrinsic_lua:IsHidden()
	return true
end

function modifier_jakiro_liquid_fire_intrinsic_lua:IsDebuff()
	return false
end

function modifier_jakiro_liquid_fire_intrinsic_lua:IsPurgable()
	return false
end

function modifier_jakiro_liquid_fire_intrinsic_lua:GetAttributes()
...
function modifier_jakiro_liquid_fire_intrinsic_lua:OnRefresh( kv )
end

function modifier_jakiro_liquid_fire_intrinsic_lua:OnDestroy( kv )

end

--------------------------------------------------------------------------------
-- Modifier Effects
function modifier_jakiro_liquid_fire_intrinsic_lua:DeclareFunctions()
	local funcs = {
...
function modifier_jakiro_liquid_fire_intrinsic_lua:OnAttack( params )
...
function modifier_jakiro_liquid_fire_intrinsic_lua:GetModifierProcAttack_Feedback( params )
	if self.records[params.record] then
		-- apply the effect
		if self:GetAbility().OnOrbImpact then self:GetAbility():OnOrbImpact( params ) end
	end
end
function modifier_jakiro_liquid_fire_intrinsic_lua:OnAttackFail( params )
	if self.records[params.record] then
		-- apply the fail effect
		if self:GetAbility().OnOrbFail then self:GetAbility():OnOrbFail( params ) end
	end
end
function modifier_jakiro_liquid_fire_intrinsic_lua:OnAttackRecordDestroy( params )
	-- destroy attack record
	self.records[params.record] = nil
end

function modifier_jakiro_liquid_fire_intrinsic_lua:OnOrder( params )
...
function modifier_jakiro_liquid_fire_intrinsic_lua:GetModifierProjectileName()
...
--------------------------------------------------------------------------------
-- Helper
function modifier_jakiro_liquid_fire_intrinsic_lua:ShouldLaunch( target )
	-- check autocast
...
--------------------------------------------------------------------------------
-- Helper: Flags
function modifier_jakiro_liquid_fire_intrinsic_lua:FlagExist(a,b)--Bitwise Exist
	local p,c,d=1,0,b
...
function modifier_jakiro_liquid_fire_intrinsic_lua:GetModifierOverrideAbilitySpecial(data)
	if data.ability and data.ability == self:GetAbility() then
		...
		if data.ability_special_value == "attack_range" then
			return 1
		end
	end
	return 0
...
function modifier_jakiro_liquid_fire_intrinsic_lua:GetModifierOverrideAbilitySpecialValue(data)
	if data.ability and data.ability == self:GetAbility() then
		if data.ability_special_value == "damage" then
			local damage = self:GetAbility():GetLevelSpecialValueNoOverride( "damage", data.ability_special_level )
            if self:GetCaster():FindAbilityByName("npc_dota_hero_jakiro_int6") then
                damage = damage + self:GetCaster():GetIntellect() * 0.2
            end
            return damage
		end
		if data.ability_special_value == "radius" then
		...
		if data.ability_special_value == "attack_range" then
			local attack_range = self:GetAbility():GetLevelSpecialValueNoOverride( "attack_range", data.ability_special_level )
