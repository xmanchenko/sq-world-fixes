...
function modifier_jakiro_macropyre_lua_thinker:OnCreated( kv )
	...
	self.range = self:GetAbility():GetCastRange( self:GetParent():GetAbsOrigin(), nil ) + self.caster:GetCastRangeBonus()
	...
	self.startpoint = self:GetParent():GetOrigin() + self.direction * start_range
...
function modifier_jakiro_macropyre_lua_thinker:OnIntervalThink()
	...
	local enemies = FindUnitsInLine(
		...
		self:GetAbility()TargetTeam,	-- int, team filter
		self:GetAbility()TargetType,	-- int, type filter
		self:GetAbility()TargetFlags	-- int, flag filter
	)
