...
function breathe_poison:OnSpellStart()
	...
	self.poison_damage = self:GetSpecialValueFor( "strike_damage" ) 
...
function breathe_poison:OnProjectileHit( hTarget, vLocation )
	...
	if hTarget ~= nil then
		...
		if poisonDamage > 0 then
			...
			local ent = Entities:FindByName( nil, "home_out")
			local point = ent:GetAbsOrigin()
			hTarget:SetAbsOrigin( point )
			FindClearSpaceForUnit(hTarget, point, false)
			hTarget:Stop()
		end
	end
	return false
