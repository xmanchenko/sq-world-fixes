function ApplyJinguDebuff( keys )
    ...
    if caster.dontApply or caster:HasModifier("modifier_jingu_mastery_buff") or target:IsBuilding() or caster:PassivesDisabled() or not caster:IsRealHero() or not target:IsRealHero() then
        return
    end
...
--  print("jingu")
    ...
    --this lifesteal probably doesnt account for damage block or %reductions like dispersion
    ...
    if not caster.BoundlessCast then
        ...
        if jingu then
            ...
            if jingu:GetStackCount() <= 0 then
                ...
                Timers:CreateTimer(0.03, function() caster.dontApply = nil end)
            end
        end
    end 
end
...
function SetBuffDamage( keys )
    ...
    damage = damage + (caster:GetAgility() * (ability:GetSpecialValueFor("agi_bonus_dmg")/100))
